...
const unwrap = response => {
  if (response?.success === false || response?.isSuccess === false) {
    throw new Error('The category request could not be completed.');
  }
...
export const categoryApi = {
  ...
  setCategoryStatus: async (id, isActive) => unwrap(await apiClient.patch(API_ENDPOINTS.CATEGORIES.STATUS(id), null, { params: { isActive } })),
};
