...
export function unwrapCustomerResponse(response) {
  ...
  if (envelope.success === false || envelope.isSuccess === false)
    throw new Error(
      safeCustomerMessage(envelope.message) ||
        "The customer request was rejected."
    );
  return Object.keys(object(envelope.data)).length
...
export function customerQuery(params) {
  if (
    ...
    params.page < 1 ||
    ![10, 25, 50, 100].includes(params.pageSize)
  )
    throw new Error("Invalid customer page or page size.");
  if (params.status && !["active", "inactive"].includes(params.status))
    throw new Error("Invalid customer status filter.");
  if (
    params.customerType &&
    !["Business", "Individual", "Organization"].includes(params.customerType)
  )
    throw new Error("Invalid customer type filter.");
  if (
    params.taxRegistration &&
    !["Registered", "Unregistered"].includes(params.taxRegistration)
  )
    throw new Error("Invalid tax registration filter.");
  if (
    params.outstanding &&
  ...
  if (
    ...
    throw new Error(
      "Invalid customer sort field. Reset filters to load customers."
    );
  if (params.sortOrder && !["asc", "desc"].includes(params.sortOrder))
    throw new Error("Invalid customer sort direction.");
  return {
...
export function mapCustomer(response) {
  ...
  const row = Object.keys(object(data.customer)).length
    ? object(data.customer)
    : Object.keys(object(data.profile)).length
    ? object(data.profile)
    : data;
  ...
  if (
    ...
    throw new Error(
      "Customer API response is missing a valid numeric id/customerId. Confirm the backend response contract."
    );
  ...
  return {
    ...
    customerType: ["individual", "business", "organization"].includes(
      ...
      : "",
    currency: string(row.currency ?? object(data.financialSummary).currency),
    outstandingBalance: amount(
      object(data.financialSummary).outstandingBalance ?? row.outstandingBalance
    ),
    creditLimit: amount(row.creditLimit),
    ...
    backend: {
      ...row,
      ...(Array.isArray(data.addresses) ? { addresses: data.addresses } : {}),
    },
  };
}
...
export function mapCustomerPage(response, params) {
  ...
  if (
    ...
    throw new Error(
      "Customer API must return items and totalCount (optionally inside data). Its Swagger does not define a response schema; confirm the authenticated response."
    );
  }
  ...
  if (
    ...
    throw new Error("Customer API returned invalid pagination metadata.");
  return {
...
export function customerPayload(values, existing) {
  ...
  if (existing)
    preserved.forEach((key) => {
      if (existing.backend[key] !== undefined)
        payload[key] = existing.backend[key];
    });
  for (const key of [
    "name",
    "companyName",
