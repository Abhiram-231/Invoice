...
import {
  validCustomerId,
  safeCustomerMessage,
} from "../../../../../billing-contracts/customer.contracts.js";

export { validCustomerId };
const path = (id) => {
  if (!validCustomerId(id))
    throw Object.assign(new Error("Invalid customer ID."), {
      code: "INVALID_ID",
    });
...
export function customerError(error) {
  ...
  const messages = {
    400: "Invalid customer data. Please check the form values.",
    401: "Your session has expired. Please login again.",
    403: "You do not have permission to access customer data.",
    404: "Customer not found.",
    409: "This customer was changed by another user. Reload before saving.",
  };
  const data = error.response?.data;
  const validationMessages =
    [400, 409, 422].includes(status) && data && typeof data === "object"
      ? [data.message, ...Object.values(data.errors || {}).flat()]
          .map(safeCustomerMessage)
          .filter(Boolean)
      : [];
  const detail =
    ...
    }Customer service is currently unavailable. Please try again.`;
  const responseText =
...
async function request(operation) {
  try {
    ...
    return response && Object.hasOwn(response, "data")
      ? response.data
      : response;
  } catch (error) {
...
const address = (value) =>
  value
    ? { ...value, line1: value.addressLine1, line2: value.addressLine2 }
    : null;
export const mapCustomer = (c) => ({
  ...c,
  status:
    c.isActive === true ? "Active" : c.isActive === false ? "Inactive" : null,
...
export function mapDetails(data) {
  if (
    data == null ||
    (Object.hasOwn(data, "customer") && data.customer == null)
  )
  ...
  if (!data?.customer)
    throw new Error(
      "Customer service returned an invalid response. Please try again."
    );
  for (const rows of [
    data.addresses,
    ...
    if (
      ...
      throw new Error(
        "Customer service returned invalid records. Please try again."
      );
  }
  const addresses = [
    ...(data.addresses || []),
...
export async function getCustomers(params = {}) {
  ...
  if (!Array.isArray(data?.items))
    throw new Error(
      "Customer service returned an invalid list response. Please try again."
    );
  return { ...data, items: data.items.map(mapCustomer) };
}
export async function getCustomerById(id) {
  const url = path(id);
  return mapCustomer(await request(() => apiClient.get(url)));
}
export async function getCustomerDetails(id) {
  const url = path(id);
  return mapDetails(await request(() => apiClient.get(`${url}/details`)));
}
...
export async function getCustomerAudit(id) {
  ...
  if (!Array.isArray(rows))
    throw new Error(
      "Customer audit response is not supported. Please contact support."
    );
  const display = (value) =>
...
export const createCustomer = (payload) =>
  request(() => apiClient.post("/api/v1/customers", payload));
