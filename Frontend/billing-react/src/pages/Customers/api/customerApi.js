...
try {
  if (typeof localStorage !== "undefined")
    localStorage.removeItem("ibms.customers.demo.v1");
} catch {
  /* Browser storage may be unavailable; backend requests still work. */
}
...
async function request(operation) {
  try {
    ...
    unwrapCustomerResponse(result);
    return result;
  } catch (error) {
    if (error?.response?.status === 401)
      throw new Error("Your session is missing or expired. Sign in and retry.");
    if (error?.response?.status === 403)
      throw new Error(
        "Your account does not have access to these customer records."
      );
...
export async function getCustomers(params, signal) {
  ...
  return request(async () =>
    mapCustomerPage(
      await apiClient.get(endpoint, { ...config, params: query, signal }),
      params
    )
  );
}
export async function getCustomerById(id, signal) {
  return request(async () =>
    mapCustomer(await apiClient.get(customerPath(id), { ...config, signal }))
  );
}
export async function getCustomerDetails(id, signal) {
  return request(async () =>
    mapCustomer(
      await apiClient.get(`${customerPath(id)}/details`, { ...config, signal })
    )
  );
}
export async function createCustomer(values) {
  await request(() =>
    apiClient.post(endpoint, customerPayload(values), config)
  );
}
...
export async function activateCustomer(id) {
  ...
  await request(() =>
    apiClient.put(
      customerPath(id),
      { ...customerPayload({}, existing), status: "Active", isActive: true },
      config
    )
  );
}
...
export async function getCustomerSummary(signal) {
  return request(async () => {
    const data = unwrapCustomerResponse(
      await apiClient.get(`${endpoint}/summary`, { ...config, signal })
    );
