...
export function CustomerDetailsPage() {
  ...
  return <main className="customer-page">
    ...
    {query.isSuccess && <>
      <header className="customer-heading"><div><h1>{c.name || '—'}</h1><div className="customer-meta"><span>Customer Code: {c.customerCode || '—'}</span>{c.companyName && <span>{c.companyName}</span>}<StatusBadge value={c.status} /></div></div><div className="customer-actions"><Button component={RouterLink} to="/customers">Back to Customers</Button><Button variant="outlined" component={RouterLink} to={`/customers/${encodeURIComponent(customerId)}/edit`}>Edit Customer</Button><DeactivateCustomerDialog key={c.id} customer={c} /></div></header>
