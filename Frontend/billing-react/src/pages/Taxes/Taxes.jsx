...
const useTaxes = () => useQuery({ queryKey: ['tax-settings'], queryFn: taxService.list, retry: false });
...
function TaxForm({ initial, editing = false }) {
  ...
  const submit = async event => {
    ...
    try {
      await taxService.save(values, initial?.id);
