...
export const emptyTax = () => ({ name: '', code: '', type: '', rate: '', calculation: 'Exclusive', priority: '0', effectiveFrom: '', effectiveTo: '', status: 'Active' });
...
export function validateTax(values) {
  ...
  if (String(values.priority).trim() === '' || !Number.isSafeInteger(Number(values.priority)) || Number(values.priority) < 0) errors.priority = 'Enter a non-negative whole number.';
